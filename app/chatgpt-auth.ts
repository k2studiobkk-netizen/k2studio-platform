...
export async function getChatGPTUser(): Promise<ChatGPTUser | null> {
  ...
  const accessHost = (process.env.ADMIN_HOSTNAME ?? "").toLowerCase();
  if (accessEmail && accessHost && requestHost === accessHost) {
    return {
      userId: accessEmail,
...
export function adminSignOutPath(user: ChatGPTUser, returnTo = "/"): string {
  return user.provider === "cloudflare-access"
    ? `/cdn-cgi/access/logout?returnTo=${encodeURIComponent(returnTo)}`
